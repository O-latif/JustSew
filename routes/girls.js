...
router.get("/girls",checkCart, async (req,res) => {
    const r = await product.find({gender : 'girl'}).sort({createdAt : -1});
    const N = await product.find({gender : 'girl'}).sort({name : 1});
    let resultT = [];
    let resultN = [];
    ...
    res.render("pages/girls",{css:"" ,all : r,Tproducts : resultT,Nproducts : resultN,act : 1, cartProd : cartArr,wish : wishProducts, title : "Girls"});
})
