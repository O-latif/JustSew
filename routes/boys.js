...
router.get("/boys",checkCart, async (req,res) => {
    const r = await product.find({gender : 'boy'}).sort({createdAt : -1});
    const N = await product.find({gender : 'boy'}).sort({name : 1});
    let resultT = [];
    let resultN = [];
    ...
    res.render("pages/boys",{css:"" ,all : r,Tproducts : resultT,Nproducts : resultN,act : 1, cartProd : cartArr,wish : wishProducts, title : "Boys"});
})
